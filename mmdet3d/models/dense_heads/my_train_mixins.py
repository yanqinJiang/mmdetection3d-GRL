...
class myAnchorTrainMixin(object):
    ...
    def anchor_target_3d(self,
        ...
        (all_labels, all_label_weights, all_bbox_targets, all_bbox_weights,
         all_dir_targets, all_dir_weights, all_mydd_targets, all_mydd_weights, pos_inds_list,
        ...
        mydd_targets_list = images_to_levels(all_mydd_targets, num_level_anchors)
        mydd_weights_list = images_to_levels(all_mydd_weights, num_level_anchors)
        return (labels_list, label_weights_list, bbox_targets_list,
                bbox_weights_list, dir_targets_list, dir_weights_list,
                mydd_targets_list, mydd_weights_list, num_total_pos, num_total_neg)
    ...
    def anchor_target_3d_single(self,
        ...
        if isinstance(self.bbox_assigner,
            ...
            (total_labels, total_label_weights, total_bbox_targets,
             total_bbox_weights, total_dir_targets, total_dir_weights, total_mydd_targets, total_mydd_weights,
            ...
            for i, assigner in enumerate(self.bbox_assigner):
                ...
                if self.assign_per_class:
                    ...
                    anchor_targets = self.anchor_target_single_assigner(
                        assigner, current_anchors, gt_bboxes[gt_per_cls, :],gt_mydds[gt_per_cls],
                        gt_bboxes_ignore, gt_labels[gt_per_cls], input_meta,
                        num_classes, sampling)
                else:
                    anchor_targets = self.anchor_target_single_assigner(
                        assigner, current_anchors, gt_bboxes, gt_mydds, gt_bboxes_ignore,
                        gt_labels, input_meta, num_classes, sampling)

                (labels, label_weights, bbox_targets, bbox_weights,
                 dir_targets, dir_weights, mydd_targets, mydd_weights, pos_inds, neg_inds) = anchor_targets
                ...
                total_mydd_targets.append(
                    mydd_targets.reshape(feat_size, 1, rot_angles))
                total_mydd_weights.append(
                   mydd_weights.reshape(feat_size,1, rot_angles))
            ...
            total_mydd_targets = torch.cat(
                total_mydd_targets, dim=-2).reshape(-1)
            total_mydd_weights = torch.cat(
                total_mydd_weights, dim=-2).reshape(-1)
            ...
            for i, assigner in enumerate(self.bbox_assigner):
                ...
                total_mydd_targets.append(mydd_targets)
                total_mydd_weights.append(mydd_weights)
                total_pos_inds.append(pos_inds)
                total_neg_inds.append(neg_inds)
            ...
            total_mydd_targets = torch.cat(total_mydd_targets, dim=0)
            total_mydd_weights = torch.cat(total_mydd_weights, dim=0)
    ...
    def anchor_target_single_assigner(self,
        ...
        mydd_targets = anchors.new_zeros(num_valid_anchors, dtype=torch.float) # note
        mydd_weights = anchors.new_zeros(num_valid_anchors, dtype=torch.float)
        if len(gt_bboxes) > 0:
        ...
        if len(pos_inds) > 0:
            ...
            mydd_targets[pos_inds] = gt_mydds[sampling_result.pos_assigned_gt_inds]
            mydd_weights[pos_inds] = 1.0
        ...
        return (labels, label_weights, bbox_targets, bbox_weights, dir_targets,
                dir_weights, mydd_targets, mydd_weights, pos_inds, neg_inds)
