...
class ObjectSample(object):
    ...
    def __call__(self, input_dict):
        ...
        gt_distance = input_dict['gt_distance']
        gt_density = input_dict['gt_density']
        ...
        if sampled_dict is not None:
            ...
            sampled_gt_distance = sampled_dict['gt_distance']
            sampled_gt_density = sampled_dict['gt_density']

            gt_labels_3d = np.concatenate([gt_labels_3d, sampled_gt_labels],
                                          axis=0)
            gt_distance = np.concatenate([gt_distance, sampled_gt_distance], axis=0)
            gt_density = np.concatenate([gt_density, sampled_gt_density], axis=0)
        ...
        input_dict['gt_distance'] = gt_distance
        input_dict['gt_density'] = gt_density
        return input_dict
...
class ObjectNoise(object):
    ...
    def __call__(self, input_dict):
        ...
        center = input_dict['gt_bboxes_3d'].center.numpy()
        gt_distance = np.linalg.norm(center, axis=1)
        input_dict['gt_distance'] = gt_distance
...
class GlobalRotScaleTrans(object):
    ...
    def _trans_bbox_points(self, input_dict):
        ...
        if 'gt_distance' in input_dict:
            center = input_dict['gt_bboxes_3d'].center.numpy()
            distance = np.linalg.norm(center, axis=1)
            input_dict['gt_distance'] = distance
    ...
    def _scale_bbox_points(self, input_dict):
        ...
        if 'gt_distance' in input_dict:
            input_dict['gt_distance'] *= scale
        if 'gt_density' in input_dict:
            input_dict['gt_density'] /= scale**2
...
class ObjectRangeFilter(object):
    ...
    def __call__(self, input_dict):
        ...
        gt_distance = gt_distance[mask.numpy().astype(np.bool)]
        gt_density = gt_density[mask.numpy().astype(np.bool)]
