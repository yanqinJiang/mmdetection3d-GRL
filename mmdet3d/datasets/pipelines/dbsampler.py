...
class DataBaseSampler(object):
    ...
    def sample_all(self, gt_bboxes, gt_labels, img=None):
        ...
        sampled = []
        sampled_gt_bboxes = []
        sampled_gt_distances = []
        sampled_gt_densities = []
                ...
                if len(sampled_cls) > 0:
                    if len(sampled_cls) == 1:
                        sampled_gt_box = sampled_cls[0]['box3d_lidar'][
                            np.newaxis, ...]
                        sampled_gt_distance = sampled_cls[0]['gt_distance'][
                            np.newaxis, ...]
                        sampled_gt_density = sampled_cls[0]['gt_density'][
                            np.newaxis, ...]
                    else:
                        sampled_gt_box = np.stack(
                            [s['box3d_lidar'] for s in sampled_cls], axis=0)
                        sampled_gt_distance = np.stack(
                            [s['gt_distance'] for s in sampled_cls], axis=0)
                        sampled_gt_density = np.stack(
                            [s['gt_density'] for s in sampled_cls], axis=0)

                    sampled_gt_bboxes += [sampled_gt_box]
                    sampled_gt_distances += [sampled_gt_distance]
                    sampled_gt_densities += [sampled_gt_density]
        ...
        if len(sampled) > 0:
            ...
            sampled_gt_distances = np.concatenate([s for s in sampled_gt_distances], axis=0)
            sampled_gt_densities = np.concatenate([s for s in sampled_gt_densities], axis=0)
