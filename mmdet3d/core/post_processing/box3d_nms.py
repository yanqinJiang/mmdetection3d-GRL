...
def box3d_multiclass_nms(mlvl_bboxes,
    ...
    for i in range(0, num_classes):
        ...
        _mlvl_mydd_scores = mlvl_mydd_scores[cls_inds, :]
        bboxes.append(_mlvl_bboxes[selected])
        mydd_labels.append(torch.argmax(_mlvl_mydd_scores[selected], dim=1)) ##max
    ...
    if bboxes:
        ...
        mydd_labels = torch.cat(mydd_labels, dim=0)
        if mlvl_dir_scores is not None:
        ...
        mydd_labels = mlvl_mydd_scores.new_zeros((0, ), dtype=torch.long)
        dir_scores = mlvl_scores.new_zeros((0, ))
    return bboxes, scores, labels, dir_scores, mydd_labels
